...
export const BioMeshCanvas: React.FC<BioMeshCanvasProps> = ({
  ...
  useEffect(() => {
    ...
    let strandAGeometry: THREE.BufferGeometry | null = null;
    let strandBGeometry: THREE.BufferGeometry | null = null;
    let particleGeo: THREE.BufferGeometry | null = null;
    let sphereGeo: THREE.SphereGeometry | null = null;
    let sphereMatA: THREE.MeshBasicMaterial | null = null;
    let sphereMatB: THREE.MeshBasicMaterial | null = null;
    let rungMaterial: THREE.LineBasicMaterial | null = null;
    let lineMatA: THREE.LineBasicMaterial | null = null;
    let lineMatB: THREE.LineBasicMaterial | null = null;
    ...
    try {
      ...
      strandAGeometry = new THREE.BufferGeometry();
      strandBGeometry = new THREE.BufferGeometry();
      const strandAPositions: number[] = [];
      const strandBPositions: number[] = [];

      rungMaterial = new THREE.LineBasicMaterial({
        color: 0x38bdf8,
        transparent: true,
        opacity: 0.35,
      });

      sphereGeo = new THREE.SphereGeometry(0.55, 12, 12);
      sphereMatA = new THREE.MeshBasicMaterial({ color: new THREE.Color(helixColorA) });
      sphereMatB = new THREE.MeshBasicMaterial({ color: new THREE.Color(helixColorB) });
      ...
      lineMatA = new THREE.LineBasicMaterial({ color: new THREE.Color(helixColorA), transparent: true, opacity: 0.8 });
      lineMatB = new THREE.LineBasicMaterial({ color: new THREE.Color(helixColorB), transparent: true, opacity: 0.8 });
      ...
      console.warn('[BioMeshCanvas] WebGL context unavailable or disabled:', err);
    }
